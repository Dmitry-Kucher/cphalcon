...
PHP_METHOD(Phalcon_Http_Request_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&name, file, SS("name"), 0 TSRMLS_CC)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "defined", NULL, 225, &_0$$3);
		zephir_check_call_status();
		if (zephir_is_true(_1$$3)) {
			ZEPHIR_SINIT_VAR(_2$$4);
			ZVAL_LONG(&_2$$4, 4);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "pathinfo", NULL, 73, name, &_2$$4);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_LONG(&_0, 16);
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 226, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("", 1);
	}
	_1 = zephir_fetch_nproperty_this(this_ptr, SL("_tmp"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 227, finfo, _1);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 228, finfo);
	zephir_check_call_status();
	RETURN_CCTOR(mime);
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_uploaded_file", NULL, 229, tmp);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 230, _0, destination);
	zephir_check_call_status();
	RETURN_MM();
