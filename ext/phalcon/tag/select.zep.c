...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$24, "_optionsfromresultset", &_9, 412, options, using, value, _11$$24);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$26, "_optionsfromarray", &_13, 413, options, value, _15$$26);
			zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (Z_TYPE_P(optionText) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_5$$4, "_optionsfromarray", &_6, 413, optionText, value, closeOption);
			zephir_check_call_status();
